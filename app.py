...
if st.button("Analyze Job Posting", use_container_width=True, type="primary"):
    with st.spinner('Yewo is analyzing the text... Please wait.'):
        if not st.session_state.job_title or not st.session_state.job_desc:
            ...
            full_text = st.session_state.job_title + ' ' + st.session_state.job_desc + ' ' + st.session_state.job_requirement
            ...
            input_data = {
                'job_desc_length': [len(st.session_state.job_desc)], 'company_desc_length': [len(st.session_state.company_desc)],
                'percent_caps': [sum(1 for c in st.session_state.job_desc if c.isupper()) / (len(st.session_state.job_desc) + 1)],
                'exclamation_count': [st.session_state.job_desc.count('!')], 'scam_score': [scam_score],
                'employment_type': [employment_type], 'department': [department],
                'has_company_name': [1 if len(st.session_state.company_name) > 3 else 0],
                'has_company_desc': [1 if len(st.session_state.company_desc) > 10 else 0],
                'has_job_requirement': [1 if len(st.session_state.job_requirement) > 10 else 0]
            }
            ...
            if is_scam_nigerian:
                ...
                with st.expander("See Detailed Reasoning"):
                    ...
                    if len(st.session_state.job_desc) < 150 and not reasons:
