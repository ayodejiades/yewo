...
with col1:
    ...
    job_title = st.text_input("Job Title", placeholder="e.g., Marketing Manager", key="job_title")
    job_desc = st.text_area("Job Description", placeholder="e.g., We are looking for a skilled professional...", height=150, key="job_desc")
    job_requirement = st.text_area("Job Requirements", placeholder="e.g., 5+ years of experience with Python...", height=150, key="job_requirement")

with col2:
    st.subheader("Company Details")
    company_name = st.text_input("Company Name", placeholder="e.g., Dangote Group", key="company_name")
    company_desc = st.text_area("Company Profile / Description", placeholder="e.g., A technology company solving payments problems...", height=150, key="company_desc")
    ...
    with st.spinner('Yewo is analyzing the text... Please wait.'):
        if not job_title or not job_desc:
            ...
            if is_scam_nigerian:
                ...
                st.error(f"**Nigerian Model Confidence: {nigerian_proba:.0%}**")
                ...
                st.success(f"**Nigerian Model Confidence: {legit_probability:.0%}**")
                with st.expander("See Detailed Reasoning"):
