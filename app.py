...
with col1:
    ...
    job_desc = st.text_area("Job Description", placeholder="e.g., We are looking for a skilled professional to join our team...", height=150)
...
st.write("---")
if nigerian_model and global_model:
    if st.button("Analyze Job Posting for Scams", use_container_width=True, type="primary"):
        if not job_title or not job_desc:
            ...
            if is_scam_nigerian:
                ...
                st.success(f"**Low Risk: Appears Legitimate.** (Nigerian Model Confidence: {legit_probability:.0%})", icon="✅")
